...
1. **Where the second step lives.** A dialog over the page; the page itself, with the controls
   replaced in place by the confirmation; or a bar fixed to the bottom of a phone screen that the
   confirmation grows out of. `DESIGN.md` says to exhaust inline alternatives before a modal — which
   is an argument, not an answer, and the variants are how it is settled.
2. **Whether declining asks first.** It is terminal too, and it crosses nothing — so one tap
   confirmed afterwards, or the same two steps accepting takes.
3. **Where his name sits.** Above the terms, as the note's sender, or below them, beside the
