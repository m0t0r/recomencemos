...
/**
 * The space inside _p. m._, which this product chooses rather than inherits.
 *
 * The voice guide fixes the format as `3:40 p. m.` — "with the periods and the
 * space" — and the point of that space is that a line break may not fall inside
 * the abbreviation. `U+00A0` is what makes that true.
 */
 ...
 * _8:40 p. m._
 *
 * **Built from the parts rather than from `format()`, because `format()` does
 * not render a stable string across ICU versions.** CLDR moved the space inside
 * the Spanish `p. m.` abbreviation from `U+00A0` to `U+202F`, and V8 flattens
 * `U+202F` to an ordinary `U+0020` on its way out of `format()`. So the same
 * call returns a no-break space on ICU 77 and a *breakable* one on ICU 78 —
 * which silently loses the one property the voice guide asks for, in a build
 * nobody changed. `.nvmrc` says `24`, so both ICU versions are in range for this
 * repo at once; CI met 78 while this branch was written against 77.
 *
 * So the separators are named here instead: `U+00A0` inside the day period, an
 * ordinary space in front of it. The second is deliberate too — it is what
 * `format()` already rendered on both versions, so the visible output is
 * unchanged and only the guarantee is new.
 *
 * `\s` is the right matcher because it spans exactly the drift: `U+0020`,
 * `U+00A0` and `U+202F` are all whitespace, and no other part of the time
 * carries any.
 */
export function atTime(date: Date): string {
  return timeFormat
    .formatToParts(date)
    .map((part) => part.value.replace(/\s/gu, part.type === "dayPeriod" ? NO_BREAK_SPACE : " "))
    .join("");
}
