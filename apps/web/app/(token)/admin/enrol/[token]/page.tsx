...
/* PROTOTYPE — these three imports go with the block at the foot of this file. */
import { readVariant } from "./_prototype/keys";
import { PrototypeSwitcher } from "./_prototype/switcher";
import { VariantB, VariantC } from "./_prototype/variants";
...
export default async function AdminEnrolPage({
  ...
  /* PROTOTYPE — read only by the `?variant=` switcher below, and thrown away with it. */
  ...
  /* PROTOTYPE — the whole block below reverts to `<Enrolment {...secrets} />`. */
  const requested = (await searchParams).variant;
  const variant = readVariant(typeof requested === "string" ? requested : undefined);

  return (
    <>
      {variant === "A" ? <Enrolment {...secrets} /> : null}
      {variant === "B" ? <VariantB {...secrets} /> : null}
      {variant === "C" ? <VariantC {...secrets} /> : null}
