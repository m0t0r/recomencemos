 ...
 * than incidental.** The RAE prescribes it so a line break cannot split the
 * abbreviation. The voice guide asks for "the periods and the space" and this is
 * that space; spelled as an escape so the assertion cannot be "fixed" by typing
 * an ordinary one.
 *
 * **It is `format.ts` that guarantees it, not ICU — which is why this assertion
 * is worth keeping.** An earlier version of this comment said ICU prescribed
 * U+00A0, and that turned out to hold only on the ICU that happened to be
 * installed. CLDR has since moved the separator to U+202F, which V8 then
 * flattens to an ordinary space on the way out of `Intl`'s `format()`; the
 * abbreviation became breakable on ICU 78 with nothing in this repo changed, and
 * these three cases are what caught it. `atTime` now names the separator itself,
 * so what follows pins a decision rather than a vendor's current default.
 */
const PM = "p.\u00A0m.";
const AM = "a.\u00A0m.";
