...
/**
 * Where an answer lands. `redirect` throws a framework interrupt that
...
function settle(
  ...
  action: "acceptOffer" | "declineOffer",
): never {
  if (outcome.ok) redirect(`/offers/${offerId}?answered=${answered}`);
  if (outcome.reason === "already_answered") redirect(`/offers/${offerId}?answered=already`);
...
export const acceptOffer = accountActionClient
  ...
  .stateAction(async ({ bindArgsParsedInputs: [offerId], ctx }) => {
    ...
    return settle(outcome, offerId, "accepted", "acceptOffer");
  });
...
export const declineOffer = accountActionClient
  ...
  .stateAction(async ({ bindArgsParsedInputs: [offerId], ctx }) => {
    ...
    return settle(outcome, offerId, "declined", "declineOffer");
  });
