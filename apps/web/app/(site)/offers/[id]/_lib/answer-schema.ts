...
/**
 * Declining asks the same question and does not insist on the answer: it
 * crosses nothing, so whether it takes one step or two is the surface's call.
 */
export const declineInputSchema = z.preprocess(
  fromAnswerFormData,
  z.object({ confirmed: z.boolean().default(false) }),
);
