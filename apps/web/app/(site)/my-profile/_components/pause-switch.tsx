 ...
 * with the switch's semantics, and the track and thumb are drawn with the
 * light-theme tokens `switch.tsx` uses: the registry's look, on the one element
 * that can carry a form post. Its `dark:` variants are left out on purpose —
 * `DESIGN.md` gives this product one ground and forbids `dark:` overrides.
 * `buttonVariants` on a plain element is this repository's own rule for the
 * same reason — see `sign-in-link.tsx`.
 *
