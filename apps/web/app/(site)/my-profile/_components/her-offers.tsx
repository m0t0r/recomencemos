...
/**
 * **Closed is the domain's word, terminal.** Every Offer here already passed the
 * received-state whitelist, and of those every state but `delivered` is
 * terminal — so the policy's own predicate is the test, and the guard only tells
 * the type system what that means for the labels below.
 */
function isClosed(offer: ReceivedOffer): offer is ReceivedOffer & { state: ClosedState } {
  return isTerminalOfferState(offer.state);
}
