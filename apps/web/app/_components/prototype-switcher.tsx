 ...
 * one is locked.
 *
 * **It sits at the top right rather than centred at the bottom** (#181): one of
 * the compositions it switches between pins the preview and the submit to the
 * bottom of the viewport, and a floating bar over the button being compared is
 * a bar that changes what the comparison shows. `&clean=1` on the URL removes
 * it entirely, which is what a screenshot is taken with.
 */
...
export function PrototypeSwitcher({ variants, current }: PrototypeSwitcherProps) {
  ...
  return (
    <div
      ...
      className="bg-foreground text-background fixed top-4 right-4 z-50 flex items-center gap-3 rounded-full px-4 py-2 font-mono text-sm shadow-lg"
    >
