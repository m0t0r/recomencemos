...
- `//#test:gates` — `gate-test.sh`, the 313 cases that drive the repo's own gates. Ninety-three are the stage hooks; the rest drive the four gates that are not hooks at all, which are the same kind of thing — repo logic deciding whether work may proceed, so a test suite and not a script to remember to run. Its `inputs` cover `.claude/hooks/**` and every script it drives, which is the part to keep in step: a script added here and not there is one whose change a cache hit replays a pass for.
  - **Three of the scripts it drives are not gates**, and the reason is worth knowing before adding more: `scripts/ui-proof.mjs` publishes recorded proof, `scripts/first-load-bytes.mjs` measures, and `scripts/dev-origin.mjs` answers where this tree's dev server is — none of them decides whether work may proceed. They are driven here anyway because they are repo logic living in `scripts/`, and the alternative was a second test runner for one file. Taking `ui-proof.mjs` as the worked example: Every case runs `--dry-run`, which is offline by construction — no pull request, no markdown renderer, no credential — so what is under test is the part that decides _what would be published_: the naming rule, the grouping into comparisons, and the two prefixes that carry the two lifetimes. The single call that writes to the object store lives in `scripts/ui-proof-store.mjs` and is a dynamic import, so this path never loads it.
