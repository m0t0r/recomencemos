...
- `//#test:gates` — `gate-test.sh`, the 280 cases that drive the repo's own gates. Ninety-three are the stage hooks; the rest drive the four gates that are not hooks at all, which are the same kind of thing — repo logic deciding whether work may proceed, so a test suite and not a script to remember to run. Its `inputs` cover `.claude/hooks/**`, all four scripts, and `scripts/ui-proof.mjs`.
  - The last twenty-nine are the odd ones out, and the reason is worth knowing before adding more: **`scripts/ui-proof.mjs` is not a gate.** It publishes recorded proof rather than deciding whether work may proceed. It is driven here anyway because it is repo logic living in `scripts/`, and the alternative was a second test runner for one file. Every case runs `--dry-run`, which is offline by construction — no pull request, no markdown renderer, no credential — so what is under test is the part that decides _what would be published_: the naming rule, the grouping into comparisons, and the two prefixes that carry the two lifetimes. The single call that writes to the object store lives in `scripts/ui-proof-store.mjs` and is a dynamic import, so this path never loads it.
