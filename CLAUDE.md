...
- `//#test:gates` — `gate-test.sh`, the cases that drive the repo's own gates. About a quarter are the stage hooks; the rest drive the gates that are not hooks at all, which are the same kind of thing — repo logic deciding whether work may proceed, so a test suite and not a script to remember to run. Its `inputs` cover `.claude/hooks/**`, every script it drives and the package source such a script imports (`packages/errors/src/**`, for `dev-origin.mjs`'s environment reader, and `packages/domain/src/config.ts` and `user-messages.ts` with the two manifests that resolve their imports, for `database-url-form.mjs`'s connection-string resolver), and the lint configuration `lint.sh` exercises (every `.oxlintrc.json`, `package.json` for the pinned versions, `pnpm-lock.yaml`), which is the part to keep in step: a script added here and not there is one whose change a cache hit replays a pass for.
...
no build reads), and a secret goes in `passThroughEnv` on the one task that needs it. **`ENVIRONMENT`
is the one build-read variable that is a pass-through**, by [ADR-0022](docs/adr/0022-the-deployment-environment-is-named-not-inferred.md):
the image build is the only place it is set, so a local build replayed from cache after changing it
keeps the headers it was built with — `turbo run build --force` is the way past that.
...
- **React is imported one way: `import * as React from "react"`**, and reached as `React.useState`, `React.ReactNode`, `<React.Suspense>` — no named import, no type import, no default import, no re-export. `pnpm lint` refuses the rest through `repo/react-namespace-import`, this repository's own rule in `scripts/oxlint-plugin.mjs`; nothing off the shelf says it, because oxlint's `no-restricted-imports` refuses the namespace import too once given a name list. `oxlint --fix` rewrites a file into the shape, references included, and `.claude/hooks/tests/lint.sh` pins both the refusal and the fix.
