...
**Server-only is enforced twice, and which guard goes where is not a preference.** A module in `@repo/domain` that only ever runs inside Next — the connection, the health check — carries `import "server-only"`, the marker package that resolves to an empty module under the `react-server` export condition and to a bare `throw` under every other, so a client module importing it fails the **build**. A module that also runs outside Next carries only `assertServerOnly()` from `#server-only`. The dividing line is verified rather than assumed: plain `node` sets **no** `react-server` condition, so `server-only` throws there — and plain `node` is what `packages/domain/src/migrate/cli.ts` is in a Fly `release_command`, and what every Node-environment Vitest file is. Route Handlers **are** inside the `react-server` layer (checked: `pnpm build` compiles and `GET /api/health` answers 200 with the marker in place). The root `.oxlintrc.json` allows `server-only` in `import/no-unassigned-import` for this, the same way it already allows `**/*.css` — that is the rule's own escape hatch, not a relaxation of `--max-warnings 0`.

**Neither guard is the mechanism.** The `exports` map is, and it is the only one of the three with a test.
