...
run_weigh_in() { # name app expect-exit expect-grep [extra arguments]
  local name="$1" app="$2" expect="$3" want="$4" out code
  shift 4
  out=$(node "$WEIGH" --app "$app" "$@" 2>&1)
  code=$?
  if [ "$code" = "$expect" ] && printf '%s' "$out" | grep -qE "$want"; then
    pass=$((pass+1)); sec_pass=$((sec_pass+1))
    [ "$VERBOSE" = 1 ] && printf '  ok   %-51s -> exit %s\n' "$name" "$code"
...
run_origin() { # name expect-exit want-stdout want-stderr app [extra arguments]
  local name="$1" expect="$2" want_out="$3" want_err="$4" app="$5" out err code ok=1
  shift 5
  err="$ROOT/origin.err"
  out=$(node "$ORIGIN" --app "$app" "$@" 2>"$err")
  code=$?
  [ "$code" = "$expect" ] || ok=0
  if [ "$want_out" = "EMPTY" ]; then
    [ -z "$out" ] || ok=0
  else
    printf '%s' "$out" | grep -qE -- "$want_out" || ok=0
  fi
  if [ "$want_err" = "EMPTY" ]; then
    [ ! -s "$err" ] || ok=0
  else
    grep -qE -- "$want_err" "$err" || ok=0
  fi
  if [ "$ok" = 1 ]; then
    pass=$((pass+1)); sec_pass=$((sec_pass+1))
    [ "$VERBOSE" = 1 ] && printf '  ok   %-51s -> exit %s\n' "$name" "$code"
    ...
    fail=$((fail+1)); sec_fail=$((sec_fail+1)); show_header
    printf '  FAIL %-51s -> exit %s (want %s, out /%s/, err /%s/)\n' \
      "$name" "$code" "$expect" "$want_out" "$want_err"
    printf '       out: %s\n' "${out:-<empty>}"
    printf '       err: %s\n' "$(cat "$err")"
  fi
  return 0
...
run_merge() { # name expect-exit want-stdout want-stderr repo [extra arguments]
  local name="$1" expect="$2" want_out="$3" want_err="$4" repo="$5" out err code ok=1
  shift 5
  err="$ROOT/merge.err"
  out=$(node "$MERGE" --root "$repo" "$@" 2>"$err")
  code=$?
  [ "$code" = "$expect" ] || ok=0
  if [ "$want_out" = "EMPTY" ]; then
    [ -z "$out" ] || ok=0
...
run_written() { # name repo pattern
  ...
  if grep -qE -- "$pattern" "$repo/coverage/coverage-summary.json" 2>/dev/null; then
    pass=$((pass+1)); sec_pass=$((sec_pass+1))
    [ "$VERBOSE" = 1 ] && printf '  ok   %-51s -> written\n' "$name"
  else
    fail=$((fail+1)); sec_fail=$((sec_fail+1)); show_header
    printf '  FAIL %-51s -> /%s/ not in the merged summary\n' "$name" "$pattern"
  fi
  return 0
}
...
if grep -q '/r/b.ts' "$TWO/coverage/coverage-final.json" 2>/dev/null; then
  pass=$((pass+1)); sec_pass=$((sec_pass+1))
else
  fail=$((fail+1)); sec_fail=$((sec_fail+1)); show_header
  printf '  FAIL %-51s -> the second workspace is missing\n' "coverage-final.json is a union as well"
fi
