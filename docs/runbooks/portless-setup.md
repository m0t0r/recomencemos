...
| Running `pnpm admin:enrol`                   | Nothing. It is not a child of the proxy, so it asks the proxy where this tree's server answers and builds the setup link against that. `pnpm dev:origin` prints the same answer if you want to see it first, and an explicitly set `PORTLESS_URL` still wins |
| A setup link that points at `localhost:3000` | The proxy has no route for this tree, so the configured `BETTER_AUTH_URL` stood — which is correct under `PORTLESS=0` and wrong otherwise. `pnpm dev:origin` names the hostname it looked for; compare it with `portless list`                               |
