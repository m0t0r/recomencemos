...
2. **Sentry's retained events** — 30-day lookback on the Developer plan (§3). Deletion is per-issue or
   ...
   choice and `/privacy` names Sentry to the people it concerns
   (`apps/web/app/_lib/consent/processors.ts`). Both have to say so **before** an audit asks, not
   after, and anything that changes the vendor changes both.
...
| `withSentryConfig()` in `next.config.ts`                               | code-only, but a mistake there fails `pnpm build` for **every developer, CI and the deploy at once** — blast radius exceeds its class | a redeploy                |
