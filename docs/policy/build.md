...
| `ui-evidence-required`  | **Yes, where a change alters what a person sees** — a rendered surface, its copy, its focus order or its states. A change with no visible effect carries an accessibility-tree diff instead, and a change to server-only code carries neither                                                                | Whether a PR touching a rendered surface must carry a recorded artifact, or may assert the seam-3 leg in prose. Set by [ADR-0019](../adr/0019-ui-change-carries-recorded-proof-not-asserted-proof.md); the method is the `ui-proof` skill                             |
| `ui-evidence-medium`    | **Chosen by what changed, never by ticket size** — a change in _time_ (a flow, focus order, a sequence of states) is video; a change in _space_ (layout, spacing, colour, type, copy) is a before/after still pair; a change visible only in the accessibility tree is `diff snapshot` output pasted as text | Which artifact a given change owes. A rule rather than a judgement call, because size is a proxy a session can argue itself out of. The table lives in the skill so it stays beside the commands that produce each one                                                |
...
| `ui-evidence-retention` | **Review proof: 30 days past merge or close**, by the bucket's own lifecycle rule. **Story demo: kept**, under a prefix with no rule. A ticket with a spec parent produces both; a ticket from `/triage` produces review proof only                                                                          | How long each artifact lives, and what decides which is which. Expiry is a lifecycle rule and a `pull_request: closed` workflow rather than a step, because the agent stops at "PR opened" and is not present when an artifact should die                             |
...
3. Where the change touches `apps/web`, it has been verified **running** — `next-dev-loop`, not just
   ...
   **And where it alters what a person sees, that verification is recorded** and the Evidence table's
   seam-3 row links the artifact instead of narrating it. `ui-evidence-required` below is the key;
