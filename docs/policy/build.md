...
| `coverage-floor`        | **`none`** — coverage is measured on every pull request and reported on it, and it gates nothing. `.github/workflows/coverage.yml` posts the number; no Vitest config declares a `thresholds` key, and coverage is not in `required-checks`. See the section below                                                                                                                                                                                                                                                                                           | The coverage number a change may not drop below, or "none" — a real answer that stops `/tdd` asking                                                                                                                                                                                                                                                                                                                                                                |
...
### Why `coverage-floor` is `none`

It is a real answer rather than an unset one, and it is not "we have not got to it yet".

**A large part of `apps/web` is uncovered by design, and a floor would apply pressure to close
that.** Vitest cannot test `async` Server Components, and an imported Server Action is not the
compiled POST endpoint an attacker reaches — both verify at **seam 3**, against a running
`next dev`, which is why `docs/policy/build.md`'s definition of done requires that leg for any
change touching the app. A line-coverage number cannot see seam 3. It counts every route handler
and every Server Action as a gap, and the cheapest way to close a gap that is really a deliberate
architecture is to write tests against the imported function — which asserts authorization on a
code path nobody attacks, and is the shape this repository already refuses.

**The second argument is the one `docs/policy/security.md` already makes about the dependency
audit.** C7's reasoning for direct-only is that a gate red on arrival is bypassed within a week.
A floor set at today's figure is red the first time somebody deletes a well-tested module, and a
floor set below it measures nothing until the day it is wrong.

So the number is published and never enforced:

- `.github/workflows/coverage.yml` runs on `pull_request` and posts one comment, updated in place.
- It is **not** a job in `ci.yml` and **not** in `required-checks` — those six are one per entry
  so a human can require each by name, and this is deliberately not one a human should require.
- No Vitest config carries a `thresholds` key. A threshold fails the run, which is the same gate
  by another route.

**What the number is for is trend and attention**, read by a person against a specific diff:
a new module that arrived with no tests at all is visible in the per-file rows, and that is a
question for review rather than for a status check. Raising it into a gate is a policy change and
needs an ADR, not a ticket.
