...
| `csp-policy`             | **`default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self' https://accounts.google.com; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: <PHOTO_PUBLIC_BASE origin> <PHOTO_S3_ENDPOINT origin>; connect-src 'self' <sentry ingest origin> <PHOTO_S3_ENDPOINT origin>; font-src 'self'; upgrade-insecure-requests`, enforced (not report-only)**. The last directive is **conditional** — emitted only where no configured origin is plaintext, for the reason the section below gives. Shipped by `apps/web/lib/response-headers.ts`. See "The response-header set" below — every deviation from the value #232 inherited is recorded there, `'unsafe-inline'` included | The Content-Security-Policy the app ships, or an explicit decision not to ship one                                                                                                                                                                                                                                                                                                                                                                                               |
| `permissions-policy`     | **`accelerometer=(), browsing-topics=(), camera=(), geolocation=(), gyroscope=(), magnetometer=(), microphone=(), midi=(), payment=(), usb=()`** — ten named capabilities, denied. Not literally deny-all: a `Permissions-Policy` restricts only what it names, and `frame-ancestors 'none'` plus the absence of any cross-origin script is what makes the remainder acceptable. `browsing-topics` stands where an earlier draft named FLoC's `interest-cohort`, which no shipping browser has recognised since 2022                                                                                                                                                                                                                                              | Which browser capabilities a document may reach for. The photo path is a file input and not a `getUserMedia` capture, so denying the camera costs this product nothing today — and the day a surface wants one, it fails loudly in one file instead of quietly acquiring a permission nobody reviewed                                                                                                                                                                            |
...
scheme the development object store does not serve. A policy that admits an origin in one directive
and orders every request to it upgraded in another contradicts itself, and the contradiction is
settled in favour of the admission. Nothing is lost in the deployment this is for, where all three configured origins are
HTTPS.

**Development gets a different set, and the differences are the whole of it.** `'unsafe-eval'` is
added to `script-src`, because React reconstructs server-side error stacks in the browser with `eval`
in development and does not in production; HSTS and `upgrade-insecure-requests` are withheld for the
reasons above. Nothing is subtracted. `connect-src` is unchanged and deliberately carries no `ws:`
source: Turbopack's HMR socket is same-host and CSP Level 3 matches it against `'self'`, which was
