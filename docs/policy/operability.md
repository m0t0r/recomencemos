...
| `default-latency`      | **p95 ≤ 400 ms server-side, read from the request-completion line on every _routed_ request; ≤ 1200 ms user-measured, taken on a cadence by a human rather than instrumented**                          | Same, for p95 on a user-visible read path. The two halves have different instruments, and saying so is part of the value: no analytics or RUM ships, so a regression in the user-measured half is invisible between measurements |
