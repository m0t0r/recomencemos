...
| `local-database`            | **Docker Compose — Postgres 18 behind PgBouncer in transaction-pooling mode, plus MinIO as the object store.** `docker-compose.yaml` at the repo root; `pnpm db:up` starts it. The app takes the pooled connection on **6432**, migrations the direct one on **5432**, and photos an S3 endpoint on **9000**. Opt-in: tests use PGlite and CI uses neither | What `pnpm dev` connects to on a developer's machine, whether a pooler sits in front of it, and where an unreviewed photo goes. See the section below                                                                                                                                              |
...
**MinIO** on **9000**. The commands that start it are in the README, and the file that defines it is
...
test:store` — is deliberately outside `pnpm test` and outside the required checks. The moment a
required check needs a bucket, seam 2's argument has been lost.
...
exactly this class of thing. It was left open because the drifting surface was three ports and a
