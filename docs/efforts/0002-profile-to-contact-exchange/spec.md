...
| ----------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Wall card grid          | `skeleton.tsx` at the card's measured height, in the grid's own columns                | **Yes** — the load-bearing one: the standing notices sit below the grid (_amended 2026-09-12 with #276_), and a shifting grid moves them under a reader who has scrolled to them |
...
Both rows above used to argue from the notices sitting _above_ the grid, where a fallback of the
wrong height would push them down the first screen. The owner kept the block's shape — a heading and
three disclosures — and moved it: _"should be displayed at the bottom, not at the top."_ A reader who
came only to look reaches the people first, and story 11 still gets all three statements together on
every surface it names. The reason the skeleton height matters has changed, but it has not gone
away: below the grid, a fallback of the wrong height moves the notices — and on the Wall, _Cómo
funciona_ after them — once the rows arrive. On a phone that happens below the fold, so it is not a
first-screen shift, but a reader who scrolled down while the list was still streaming sees it. So the
row stays **Yes** for a different reason, and the Wall's `error` cell is unchanged: the notices sit
...
decision surfaces — the One Offer page and the Contact Exchange — which was the owner's call, not
this ticket's. `.impeccable/briefs/standing-notices.md` records the per-surface order.
