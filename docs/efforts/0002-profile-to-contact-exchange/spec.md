...
- **NFR1 — Publish latency, end to end.** A Worker completing the form reaches the Wall in **≤ 5 s**,
  ...
  **Binds:** 2.
- **NFR2 — Read latency.** p95 **≤ 400 ms** server-side for `/` and `/perfiles` on the Fly `iad`
  ...
  **Binds:** 2, 4, 19.
- **NFR3 — Worker-path page weight.** `/` and `/publicar` each ship **≤ 120 KB** of compressed
...
- **NFR8 — Gated surfaces are never indexed.** **100%** of responses for `/perfil/*`, `/propuesta*`,
  `/mi-perfil*`, `/mis-propuestas*`, `/cuenta*`, `/admin/*` carry `X-Robots-Tag: noindex, nofollow`
  and a `<meta name="robots">` equivalent, asserted by a table-driven test over the route list rather
  than a per-page attribute. `robots.txt` **does not** `Disallow` `/perfil/*`: a disallowed URL is
...
| `GET /perfiles`                   | Server Component. Fewest delivered Offers first | `PublicProfile[]`, keyset-paginated                                | Anyone. Indexable                                                                                               |
| `GET /entrar`                     | Server Component + one Client Component         | —                                                                  | Anyone                                                                                                          |
...
| `GET /robots.txt`, `/sitemap.xml` | Route Handlers                                  | —                                                                  | Anyone. The sitemap lists `/` and `/perfiles` and **no** profile                                                |

### Worker — Account required, `noindex`

| Surface                                                 | Shape                                                                                                                                 | Who may call                                                                                                                                                                                                               |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /publicar`                                         | —                                                                                                                                     | Signed-in Account with no CapabilityProfile                                                                                                                                                                                |
...
| `GET /mi-perfil`                                        | `OwnProfile` — the gated shape plus her own photo whatever its state                                                                  | The owner                                                                                                                                                                                                                  |
| `GET /propuestas`, `GET /propuesta/[id]`                | `ReceivedOffer[]`, `ReceivedOffer`                                                                                                    | The owner, scoped by profile ownership                                                                                                                                                                                     |
...
| Surface               | Shape                                                                                                | Who may call                                                                                                                      |
| --------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `GET /perfil/[slug]`  | `GatedProfile`                                                                                       | Signed-in Account not Blocked. Charged against NFR26's read ceiling. A Blocked caller gets the same response as a missing profile |
...
| `GET /mis-propuestas` | `SentOffer[]` — state only; no contact details unless exchanged                                      | The sender                                                                                                                        |
...
**Story 1 — sign in.** `/entrar` is a Server Component with one Client Component: the email field and
...
**Story 2 — publish.** `/publicar` posts every field except the photo through `publishProfile`
...
**Story 4 — browse.** `/` and `/perfiles` are Server Components reading `PublicProfile[]` through a
...
**Story 5 — read a full profile.** `/perfil/[slug]` reads the session **[trust]**, redirects an
anonymous caller to `/entrar` with a validated return path, checks the Block edge, charges the read
...
**Story 11 — the notices.** Product components on the Wall, on `/perfiles`, on every profile and every
...
`/propuesta/[id]` puts a primary key in a URL, and a `BIGINT IDENTITY` there publishes the platform's
total Offer count to every Hirer on his first Offer, and hands an enumerator a clean `/propuesta/1..N`
...
- _I create an Account and read every gated profile, harvesting the self-descriptions of every
  displaced person on the site._ `PublicProfile` carries `slug`, so `/perfiles` hands an enumerator the
...
| -------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Wall**             | `app/page.tsx`                | Before the first profile exists: the proposition and a route into `/publicar`. Never a blank region            | Card skeletons at the card's exact height, so the notices above them do not move | Cards rendered, a photo slot still resolving → the initial, which is also the approved-photo-absent state. One shape, two causes | The Wall failed to load: what failed, that retrying helps, and the notices still render                                                                                                                                                 | n/a — public                                                                                                                                             | n/a                                                                                                                                                                                         |
| **Browse**           | `app/perfiles/page.tsx`       | No match for this Skill/city: say which filter is narrowing, offer to clear it. Distinct from the Wall's empty | Skeletons holding the grid                                                       | Some cards, more streaming                                                                                                       | Search failed; the unfiltered list is still reachable                                                                                                                                                                                   | n/a — public                                                                                                                                             | n/a                                                                                                                                                                                         |
| **Full profile**     | `app/perfil/[slug]/page.tsx`  | n/a                                                                                                            | Skeleton at the profile's height                                                 | Identity rendered, work history streaming                                                                                        | Profile failed to load                                                                                                                                                                                                                  | **Signed out** → the Account gate and why it exists. **Blocked** → indistinguishable from "not found", because the alternative tells him she Blocked him | n/a                                                                                                                                                                                         |
| **Sign in**          | `app/entrar/page.tsx`         | n/a                                                                                                            | Button busy, form still readable                                                 | n/a                                                                                                                              | Send failed, retry available, and the address is still in the field                                                                                                                                                                     | n/a                                                                                                                                                      | "Check your email" — said **whether or not the address exists**, because the honest reply and the enumeration-safe reply are the same one. A consumed link offers an immediate resend (DD5) |
| **Publish**          | `app/publicar/page.tsx`       | Skill picker before a query                                                                                    | Per-field, never a whole-form spinner                                            | Fields accepted, photo still uploading — the profile is already live                                                             | Per-field errors **and** a focused form-level summary; the contact-detail rejection names the fragment and keeps everything typed (NFR12); a photo rejected at the size ceiling says so in her terms rather than failing opaquely (DD6) | Already has a profile → route to `/mi-perfil`                                                                                                            | Published, with the live profile linked and the pending photo explained without a badge                                                                                                     |
| **Own profile**      | `app/mi-perfil/page.tsx`      | n/a                                                                                                            | Skeleton                                                                         | Photo pending → **her own photo shown**, dignified, described as under review, not flagged                                       | Load failed                                                                                                                                                                                                                             | Not the owner → 404                                                                                                                                      | Edit saved                                                                                                                                                                                  |
| **Received Offers**  | `app/propuestas/page.tsx`     | No Offers yet: say what makes one arrive, and that a person reads each first                                   | Row skeletons                                                                    | Some rows, terms streaming                                                                                                       | Load failed                                                                                                                                                                                                                             | Not the owner → 404                                                                                                                                      | n/a                                                                                                                                                                                         |
| **One Offer**        | `app/propuesta/[id]/page.tsx` | n/a                                                                                                            | Skeleton at the terms' height                                                    | Terms rendered, Hirer identity streaming                                                                                         | Load failed                                                                                                                                                                                                                             | Not the addressee → 404                                                                                                                                  | Accepted → the Contact Exchange below. Declined → confirmed, and it stays confirmed rather than vanishing                                                                                   |
...
| **Sent Offers**      | `app/mis-propuestas/page.tsx` | None sent: route into `/perfiles`                                                                              | Row skeletons                                                                    | Rows, states streaming                                                                                                           | Load failed                                                                                                                                                                                                                             | Not the sender → 404                                                                                                                                     | n/a                                                                                                                                                                                         |
| **Account**          | `app/cuenta/page.tsx`         | n/a                                                                                                            | Per-action                                                                       | n/a                                                                                                                              | Action failed                                                                                                                                                                                                                           | Signed out → `/entrar`                                                                                                                                   | Signed out everywhere / email change **pending verification** (DD5) / **deletion**, whose confirmation carries NFR11's second number: it reaches nothing a Hirer already read               |
...
- [ ] **C3** — **A Block cannot mean what `CONTEXT.md` says it means.** The glossary binds Block as
      "her CapabilityProfile becomes invisible to him". The Wall and `/perfiles` are public, indexable
...
- [ ] **C9** — **The availability target for the public reads.** Proposal: **99.5% monthly** on `/` and
      `/perfiles`, measured by the external uptime monitor rather than server-side. Note the
