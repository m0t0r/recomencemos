...
- **NFR26 — Ceilings on the two exhaustible resources.** Gated profile reads **≤ 60 per Account per
  ...
  **≤ 3/day**, `requestMagicLink` **≤ 5/hour per address and ≤ 20/hour per IP**. Per Account:
  `pauseProfile` and `resumeProfile` **together ≤ 10/day**, one counter for both.
  ...
  spend each other's allowance._ One Account holds at most one CapabilityProfile, by
  ...
  **Third half, which the second was missing (C39): a refusal is legible to the person who hit it.**
  ...
  state table — **all ten ceilings, checked as a list against that table rather than by eye** (C57).
  ...
  so it is one row in that table, not two._
...
| actions `pauseProfile`, `resumeProfile`                 | `{}` → `{ ok }`. `pauseProfile` stamps `pausedAt`; `resumeProfile` clears it. **Both idempotent**: pausing a paused profile and resuming a visible one are no-ops, not errors. `publishedAt` untouched. Added 2026-09-10 with #141 — see below | The owner. Rate-limited (NFR26), **one ceiling shared by both**. While the profile is taken down, each answers `{ ok }` and writes nothing, and the surface offers neither                                                 |
...
| **Own profile**  | `pauseProfile` + `resumeProfile` ≤ 10/day together (#141) | How many times she has used the switch today, when she may use it again, and which state the profile is in now — paused or visible — so she is never left guessing whether the last tap took                                                                                                                                              |
