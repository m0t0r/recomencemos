...
| Surface                                                                                                                                                                                 | Shape                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Who may call                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| `GET /admin`                                                                                                                                                                            | `QueueItem[]` across five sources, each branch `LIMIT`-capped for display while its **count and age-of-oldest are computed over the whole branch** (C55), plus **exactly four** signals that are not queue items — the publish rate above 10/hour (C24), the duplicate-phone flag (C30), a profile filing more than 3 Reports in 7 days (C49), and a profile completing more than 5 Contact Exchanges with distinct Hirers in 7 days (C47). A fifth signal is a spec amendment | An Admin **session** (NFR14)                                                                    |
...
1. **Ten backup codes**, generated at enrolment, **printed and stored offline**. _Amended 2026-08-30
   with #96: this read "not in the password manager that also holds the password", and there is no
   password now. The rule it was reaching for survives and is sharper — **not in the mailbox**, which
   is the other factor. A manager holding both the codes and the mail account's own credential is the
   same mistake under a different name._
...
- [x] **C43** — **There is one Admin, and losing the TOTP device locks the platform's only moderator
      ...
      **Amended 2026-08-30 with #96.** The answer stands and two of its three legs changed shape, because
      the Admin door no longer has a password. (1) The codes are still ten, still printed, still offline —
      but the rule is now **keep them off the mailbox**, which is the other factor, rather than off the
      password manager, which no longer holds anything. (2) The second grant is `pnpm admin:enrol` run a
