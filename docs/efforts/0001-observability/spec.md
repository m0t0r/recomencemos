...
| `threat-model-scope`              | Per-project                       | The runbook names the three threats this effort creates: public-DSN quota exhaustion, log injection through `requestId` (closed at source: the value is always minted by this process, and never carries a character that could break out of the line), and a credential in a URL path segment reaching `context.path` verbatim (named, not closed — ADR-0006) |
...
- [x] **C15** — Where `requestId` comes from. **Answer: generated server-side, never read from an
      inbound header.** That closes the log-injection path into the pretty dev stream an agent reads.
      `trace_id` remains the correlation key, but it exists only while reporting is active, so it
      cannot be the `requestId`. **Amended by #81**: the id is per **request** rather than per error —
      `AppError` adopts the completion-line subscriber's, and mints its own only outside a request
      ([ADR-0016](../../adr/0016-one-request-id-per-request-adopted-not-minted-per-error.md)). The
      answer above is unchanged in substance: both values are minted by this process, there is still
      no constructor option an inbound header could reach, and an adopted value is revalidated for
      length **and characters** on the way in.
