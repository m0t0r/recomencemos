...
- **Five of the scripts it drives are not gates**, and the reason is worth knowing before adding more: `scripts/ui-proof.mjs` publishes recorded proof, `scripts/first-load-bytes.mjs` measures, `scripts/dev-origin.mjs` answers where this tree's dev server is, `scripts/coverage-merge.mjs` folds six workspaces' coverage reports into the one a pull request comment is built from, and `scripts/database-url-form.mjs` answers whether production will open a connection string the go-live wizard is about to stage — none of them decides whether work may proceed. The last runs `@repo/domain`'s own resolver rather than a copy of its rule, which is why `packages/domain/src/config.ts` is a `test:gates` input. They are driven here anyway because they are repo logic living in `scripts/`, and the alternative was a second test runner for one file. Taking `ui-proof.mjs` as the worked example, fifty-five cases: most of them run `--dry-run`, which is offline by construction — no pull request, no markdown renderer, no credential — so what is under test there is the part that decides _what would be published_: the naming rule, the grouping into comparisons, and the two prefixes that carry the two lifetimes. The call that writes to the object store lives in `scripts/ui-proof-store.mjs` and is a dynamic import, so a dry run never loads it.
