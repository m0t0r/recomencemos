...
**The preview UI has two doors out that are not this package's seam.** The `Send` button POSTs the rendered markup and a recipient address to `https://react.email/api/send/test`, and the toolbar's `Spam` tab POSTs the rendered `html` and `plainText` to `https://react.email/api/check-spam` — React Email's own hosted endpoints, a third party, both with **no credential**, so both work out of the box for anyone with the server running. Neither reaches `createNotifier`, so neither is stopped by `NOTIFICATIONS_KILL_SWITCH`, neither carries an idempotency key, and neither writes a `notification.sent` line.

Treat them as foreign doors: do not use `Send` to test this product's mail, and do not preview a template carrying real personal data — `magic-link.tsx` renders a token-bearing URL, and `check-spam` sends the plain-text body alongside the markup. Whether either should be blocked outright is [#76](https://github.com/m0t0r/recomencemos/issues/76), not something this package currently enforces. (The same `check-spam` call exists in the preview server's own render path, but is gated on `NEXT_PUBLIC_IS_BUILDING`, which `email build` sets and `email dev` does not — so it does not fire automatically in this loop.)
