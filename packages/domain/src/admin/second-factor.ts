 ...
 *
 * **That last clause is what keeps the two sets disjoint by shape, and it is
 * worth being exact about what it does and does not do today.** As things stand
 * {@link classifyAdminCode} anchors on six digits exactly, so an all-digit
 * backup code — eight characters — is already unambiguous without it. What the
 * guarantee buys is that the disjointness does not depend on the two lengths
 * staying different: a code shortened to six, an input trimmed to its first six
 * characters, a door that matched on a prefix, and an all-digit code becomes a
 * TOTP code. Then the door compares a printed code against a TOTP secret and
 * refuses it, on the one occasion the codes exist for.
 *
 * So it is a property of the *set* rather than a workaround for the current
 * regex, which is why it is asserted over a population here and why
 * `classifyAdminCode` is asserted against the generator's own output.
 *
...
/**
 * A typed code reduced to the characters that carry meaning: upper case, and no
 * separators.
 *
 * **The hyphen is grouping, not content.** It exists so eight characters can be
 * read off a screen at speed; a person retyping one from memory, or a manager
 * pasting one that lost its formatting, will produce `ABCDEFGH` for a code
 * stored as `ABCD-EFGH`. Refusing that is a lockout bought with no security,
 * which is the same argument the upper-casing rests on — the alphabet is upper
 * case, so neither case nor the hyphen carries any entropy to lose.
 *
 * Anything outside the alphabet goes, not just the hyphen: a space, a
 * soft-wrapped newline and a stray tab all arrive the same way and mean the
 * same nothing.
 */
function normaliseBackupCode(code: string): string {
  return code
    .toUpperCase()
    .split("")
    .filter((character) => BACKUP_CODE_ALPHABET.includes(character))
    .join("");
}
...
export function verifyBackupCode(codes: readonly string[], typed: string): BackupCodeOutcome {
  ...
  /**
   * **Both sides are normalised, and the remainder is filtered on the same
   * comparison the match was made with.** The stored form carries its hyphen and
   * the typed one may not, so comparing raw against raw is the version of this
   * that accepts a code and then fails to remove it — which would leave a used
   * code working a second time, silently.
   */
  ...
  for (const code of codes) {
    const stored = normaliseBackupCode(code);
    if (candidate.length === stored.length && constantTimeEqual(candidate, stored)) matched = true;
  }

  return {
    matched,
    remaining: matched ? codes.filter((code) => normaliseBackupCode(code) !== candidate) : codes,
  };
}
