 ...
 *
 * **Only the read is bound.** Minting and completing are the command's, over the
 * direct connection, and giving either one a pooled binding would put the grant
 * one import away from a request path.
 *
 * **Named `enrolments` rather than `adminEnrolment`**, for the reason `ceilings`
 * is not called `rateLimit`: the latter is already the table this module writes,
 * imported from `#schema` a few lines up, and two unrelated things under one name
 * in one file is a collision waiting for the first reader who greps.
 */
