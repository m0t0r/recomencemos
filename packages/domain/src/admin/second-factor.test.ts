...
describe("generateBackupCodes", () => {
  // Ten is the number a person is asked to keep, and the runbook says so; the
  // citation stays in this comment rather than in the name, which is read in CI
  // output by somebody who does not have that document open.
...
describe("verifyBackupCode", () => {
  ...
  /**
   * A person reading a code off a screen types what they see; a manager pastes
   * what it stored, which may have lost the formatting. None of these is worth a
   * lockout and none weakens the code: the alphabet is upper case and the hyphen
   * is grouping, so neither carries any entropy to lose.
   */
  it.each(["lower", "spaced", "unhyphenated", "spaced out"] as const)(
    "accepts a code the person typed %o",
    (variation) => {
      const codes = generateBackupCodes();
      ...
      const typed = {
        ...
        spaced: ` ${code} `,
        unhyphenated: code.replace("-", ""),
        "spaced out": code.replace("-", " "),
      }[variation];
  ...
  /**
   * **The remainder is filtered on the same comparison the match was made
   * with.** Accepting `ABCDEFGH` for a stored `ABCD-EFGH` and then failing to
   * remove it would leave a used code working a second time — a bug that
   * presents as nothing at all until somebody spends one code twice.
   */
  it("consumes the code however the person typed it", () => {
    const codes = generateBackupCodes();
    const code = codes[2] as string;

    const { remaining } = verifyBackupCode(codes, code.replace("-", "").toLowerCase());
