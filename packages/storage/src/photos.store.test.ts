 ...
 * `object-store-init` performs the acts a human performs against R2 at runbook
 * §3, plus a CORS rule only a browser reads — so what is exercised here is the
 * production code path against something very close to the production
 * configuration.
 *
 ...
 * one of them — which an S3 bucket policy expresses and R2 cannot: public
 * access there is a single bucket-level switch with no per-prefix ACL. So the
 * fixture was proving
...
describe("the quarantine bucket", () => {
   ...
   * on a per-prefix anonymous policy — which an S3 bucket policy can
   * state and **R2 cannot**: public access there is one switch for the whole
   * bucket. So the property
