...
**This was a "Placeholders to change" table, read by `/bootstrap` as a fresh clone's work list. Both are gone.** Every row this product has in fact answered — the project and app names, the `@repo` scope, the development database credentials, the port, the tracker, the app README, the page metadata, the showcase page, `PRODUCT.md`, this README — has been answered and deleted from the list. What is left is **this product's own open work**, and each row says who owns it. A row is deleted when it is done, not when it is inconvenient.
