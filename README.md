...
**The components and the colours have different origins, and that matters when you regenerate.** The components came from preset [`b1Z6BvKBU`](https://ui.shadcn.com/create?preset=b1Z6BvKBU) (`vega` style, `lucide` icons, Inter, pointer cursor on buttons). The **colours did not** — they were copied from [`m0t0r/workforpereira`](https://github.com/m0t0r/workforpereira/tree/dev/packages/design-system). So `pnpm dlx shadcn@latest apply <preset>` would overwrite the palette; change the ramp instead.

#### The token layers

**A private ramp, a semantic seam.** Eleven `--brand-*` shades at **hue 248** carry the whole palette, and nothing outside `globals.css` may reference them — only the semantic names below reach Tailwind through `@theme inline`, so no component can reach past the seam into a raw shade. **Swapping the brand hue is one edit**: change `248` in the eleven ramp lines.

```css
--brand-50   oklch(0.975 0.012 248)      --brand-500  oklch(0.62  0.13  248)
--brand-100  oklch(0.945 0.026 248)      --brand-600  oklch(0.545 0.13  248)   → --chart-1
--brand-200  oklch(0.885 0.057 248)      --brand-700  oklch(0.48  0.122 248)   → --primary, --ring
--brand-300  oklch(0.81  0.094 248)      --brand-800  oklch(0.41  0.109 248)
--brand-400  oklch(0.715 0.114 248)      --brand-900  oklch(0.33  0.089 248)
                                          --brand-950  oklch(0.235 0.066 248)
```

**Contrast is a property of the lightness ramp, not the hue.** Every `L` above is fixed; hue and chroma are free. `--primary` is shade **700, not 600** — 700 carries white at 6.5:1 rather than 4.9:1.

| Layer           | Tokens                                                                                | Notes                                                                                                                                                                                                                                    |
| --------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ground and ink  | `background` `foreground` `card` `card-foreground` `popover` `popover-foreground`     | Page is pure white; ink is `oklch(0.2 0.012 248)` — the brand hue at near-black, not neutral grey                                                                                                                                        |
| Brand roles     | `primary` `primary-foreground` `secondary` `accent` `muted` `muted-foreground` `ring` | `primary` and `ring` are both `--brand-700`                                                                                                                                                                                              |
| Edges           | `border` `input`                                                                      | **Two jobs, never collapse them.** A divider identifies nothing, so `border` is faint (`L 0.915`). An input's boundary is the only thing identifying the control, so `input` is `L 0.65` — WCAG 2.2 SC 1.4.11 wants 3:1 against the page |
...
| Radius          | `--radius: 0.625rem`                                                                  | `sm`…`4xl` are `calc()` multiples of it (`×0.6` to `×2.6`), so one edit rescales all seven                                                                                                                                               |
...
**The chart palette is colour-vision-safe by its ordering.** Slot 1 is `--brand-600` for cohesion with `--primary` while sitting one step off it, so a data mark never reads as a button. The other four are ≥45° apart in hue from each other and the brand, and ≥25° clear of every semantic hue so a series can never impersonate a state. On the adjacent pairlist — the gate for bars, lines and stacked segments — the worst pair is ΔE 14.1 under simulated protanopia/deuteranopia against a target of 8. **On the all-pairs list — scatter, bubble, map, small multiples — five slots cannot pass and no ordering fixes it.** Those forms carry **two** series from this palette; more means facets, or folding the tail into "Other".

#### There is no dark mode

The source repo decided against one and this repo followed, so `next-themes`, the theme provider and the toggle are gone and there is no `.dark` block.
...
**This was a "Placeholders to change" table, read by `/bootstrap` as a fresh clone's work list. Both are gone.** Every row this product has in fact answered is gone from the list. Most were answered by being changed — the project name, the development database credentials, the tracker, the app README, the page metadata, the showcase page, `PRODUCT.md`, this README. Three were answered by a **decision to keep**, which is a real answer and not a skipped row: the app is still `web`, the scope is still `@repo`, and the port is still `3000`. What is left is **this product's own open work**, and each row says who owns it. A row is deleted when it is done, not when it is inconvenient.

| Still open                                                             | Where                                                                                                                                             | What it needs, and who owns it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NFR14's refusal of every passwordless door for an Admin                | `packages/domain/src/auth/config.ts` — `databaseHooks.session.create.before`                                                                      | NFR14 requires every passwordless door — magic link and Google alike — refused for an account holding the Admin grant, and #12 shipped **neither**, because the grant does not exist as a column yet (DD7 makes the first one a documented manual `UPDATE`). NFR14 `Binds: 7, 20`, so it is those stories' work; `session.signInMethod` is already written on every session so `requireAdmin` has the field it needs. The rule is written over the **class** of passwordless doors precisely so a third one does not slip past — [#17](https://github.com/m0t0r/recomencemos/issues/17)                                                                                                                                                                                                |
| Zod's weight in the client bundle, unmeasured against NFR3             | `apps/web/app/(site)/(auth)/sign-in/_lib/schema.ts`, imported by the Client Component                                                             | [ADR-0014](docs/adr/0014-one-schema-parsed-twice-and-a-form-layer-that-survives-no-javascript.md) originally justified zod on the client as free, because `better-auth/react` was said to pull it already. **That was false** — that package's whole client graph is `@better-fetch/fetch`, `defu`, `nanostores` and `react`, with no zod — so the ADR's own conclusion is that the cost is real, unquantified, and payable against NFR3's 120 KB budget. No `/publish` figure has been taken, because `/publish` does not exist yet. Story 2 owns taking it, and the ADR says which way to resolve it: _"if `/publish` cannot meet NFR3's 120 KB budget with this layer in it, the layer goes, not the budget"_ — [#16](https://github.com/m0t0r/recomencemos/issues/16)              |
...
| The palette and the icon set                                           | `packages/design-system/src/styles/globals.css` + `components.json`                                                                               | The eleven `--brand-*` shades at hue 248 were copied from [`m0t0r/workforpereira`](https://github.com/m0t0r/workforpereira), not chosen for this product. They are **not** shadcn-preset output, so `shadcn apply` would overwrite them — edit the ramp instead. The lightness ramp was contrast-audited in the source repo by a `scripts/check-contrast.mjs` that was **not** copied here; port it before changing any `L` value                                                                                                                                                                                                                                                                                                                                                      |
| Inter / Geist Mono via `next/font/google`                              | `apps/web/app/layout.tsx`                                                                                                                         | Inherited with the preset rather than chosen for this product. The same design decision as the palette above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `DESIGN.md`'s YAML frontmatter                                         | [`DESIGN.md`](./DESIGN.md) — **the frontmatter only**                                                                                             | Moves with the palette above. The prose body is durable method and is not open                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Root-boundary palette (six hex values, twice)                          | `apps/web/app/global-error.tsx` — the `styles` string                                                                                             | Moves with the palette above. They are literal hex rather than design-system tokens because this boundary **replaces the root layout**: no stylesheet is mounted, so it cannot reach the token layer and follows the OS colour scheme through two `prefers-color-scheme` queries instead. Whatever lands here must clear `wcag-level` in **both** schemes on its own                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Repo-level things set up outside the code: a remote cache (`pnpm exec turbo login && pnpm exec turbo link`), and branch protection so an agent cannot approve its own code — requiring the five status checks `.github/workflows/ci.yml` publishes (`lint`, `check-types`, `test`, `build`, `audit`).
...
| [`.github/workflows/`](./.github/workflows/)            | **Included** — `ci.yml` runs the five required checks on every PR, `needs-triage.yml` files findings. The non-interactive eval suite is not written                                          |
