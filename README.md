...
| ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Page weight on every route, **measured** and over NFR3's budget               | The framework baseline underneath every route, `packages/design-system`'s popup machinery, and the form layer on `/publish` and `/sign-in`                                             | Re-measured after [#157](https://github.com/m0t0r/recomencemos/issues/157) with `pnpm page-weight`, which is the method rather than a recollection of it — gzip on the wire, every `<script>` the prerendered document requests, the `noModule` bundle no browser fetches excluded: `/` **219 KB**, `/profiles` **221 KB**, `/privacy` **217 KB**, `/sign-in` **330 KB**, `/publish` **341 KB** — re-taken on this build at [#184](https://github.com/m0t0r/recomencemos/issues/184), which is also where the drift finally has an attribution. #182 left `/` at 218 KB, `/profiles` at 220 KB, `/privacy` at 216 KB, `/sign-in` at 308 KB and `/publish` at 320 KB, and the **one** commit between that measurement and this one is [`31e57c1`](https://github.com/m0t0r/recomencemos/commit/31e57c1), a fourteen-package minor-and-patch bump: it costs about a kilobyte on the three list-and-prose routes and **21–22 KB on the two form routes**, which is where a dependency bump would land. `/profiles` measured identically with and without #184's own diff, on two builds — a framing change moves classnames, and classnames are CSS. #157 took **84 KB** off every route by moving the Sentry SDK behind a dynamic import and the `<Toaster />` out of the root layout, and it is the end of what those two levers have: a build with Sentry deleted outright measured 207 KB on `/`. **That 207 KB, and the three-way split at the end of this row, are #157's build and have not been re-taken since** — `/` has moved 12 KB above it in the meantime, so the figure is the floor Sentry's removal reached on that tree and not a claim about this one. It is kept because what it establishes is a _ratio_ rather than a total: deleting the SDK outright bought roughly what moving it behind a dynamic import already had. **NFR3's amended budget — `/` ≤ 140 KB, `/publish` ≤ 250 KB — is therefore not reachable and was never reachable**, because the attribution it was set from credited Sentry with 148 KB of `/`'s 292 KB when its true marginal cost was 85 KB; the rest of that chunk is code the page needs anyway. What is actually left on `/` is React and react-dom at **72 KB**, Base UI's popup stack at **63 KB** and the Next runtime at **73 KB**. Owner: the human, as a spec amendment — the budget is the thing that has to move, or Base UI and the form layer are the next levers, and neither is #157's |
...
| `PHOTO_S3_ENDPOINT` / `PHOTO_PUBLIC_BASE` as **repository variables** (unset) | GitHub → Settings → Secrets and variables → Actions → **Variables**; `.github/workflows/deploy.yml` reads both, `turbo.json` declares them on `build`                                  | Both already exist as runtime values. What is open is the **build-time** half: `apps/web/lib/response-headers.ts` builds the Content-Security-Policy and Next bakes `headers()` into `routes-manifest.json`, so an origin the build cannot see is an origin the deployed policy does not name. The symptom is silent on the server and fatal in the browser — the presigned upload blocked by `connect-src`, the Admin's review image by `img-src`. Non-secret origins, so `vars` and never `secrets`; `scripts/deploy.sh` warns on stderr when either is unset. Owned by the **repo owner**, at go-live runbook §5. [#232](https://github.com/m0t0r/recomencemos/issues/232)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| The `ui-proof` artifact store (unprovisioned)                                 | The environment: `UI_PROOF_S3_ENDPOINT`, `UI_PROOF_S3_BUCKET`, `UI_PROOF_S3_ACCESS_KEY_ID`, `UI_PROOF_S3_SECRET_ACCESS_KEY`, `UI_PROOF_PUBLIC_BASE`; captured by `pnpm ui-proof:setup` | The bucket, its two lifecycle rules and its credential — [`docs/runbooks/ui-proof-artifacts.md`](./docs/runbooks/ui-proof-artifacts.md), a human's step, because provisioning needs an account an agent does not hold. Until it exists `pnpm ui-proof publish` refuses by design and names that runbook, so **every pull request that changes what a person sees captures its stills and cannot link them** — which is the one blocking pass in [`REVIEW.md`](./REVIEW.md) that no session can satisfy on its own. Met at [#182](https://github.com/m0t0r/recomencemos/issues/182) and again at [#183](https://github.com/m0t0r/recomencemos/issues/183). Owner: the human                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
