...
| `NOTIFICATIONS_FROM` / `NOTIFICATIONS_REPLY_TO` (`hola@example.test`)  | `apps/web/.env.example`; read by `@repo/notifications/send`                                                                                       | A real, monitored address on Recomencemos' own verified sending subdomain, and a reply-to that reaches a person. Neither has a default — `senderIdentity()` throws rather than invent one — and the reply-to is **never `noreply@`**: someone who answers a notification must reach a human (DD14). `.test` is RFC 2606 reserved and resolves nowhere. The credential itself, `RESEND_API_KEY`, is absent from every `.env` file in git — `turbo.json` declares `.env*` a `build` input. Runbook §4                                    |
