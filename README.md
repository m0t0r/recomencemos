...
| Development database name, user and password (`recomencemos`)              | `docker-compose.yaml`, `docker/pgbouncer/userlist.txt`, `docker/pgbouncer/pgbouncer.ini`, `apps/web/.env.example`                                 | Your project's name, in all four. They are development-tier values on loopback, which is why they are in git at all — a real credential belongs in `docs/policy/security.md` → `secret-store`, never here. Nothing checks that the four agree. Rename **all** of them and nothing breaks — the dev database simply keeps this template's name; rename **some** and `pnpm dev` fails to connect                                  |
...
| Design tokens copied from `workforpereira`                                 | `packages/design-system/src/styles/globals.css` + `components.json`                                                                               | Your brand's, if this is not it. These are **not** shadcn-preset output, so `shadcn apply` would overwrite them — edit the `--brand-*` ramp instead. The lightness ramp was contrast-audited in the source repo by a `scripts/check-contrast.mjs` that was **not** copied here; port it before changing any `L` value                                                                                                           |
...
Repo-level things to set up outside the code: a remote cache (`pnpm exec turbo login && pnpm exec turbo link`), and branch protection so an agent cannot approve its own code — requiring the five status checks `.github/workflows/ci.yml` publishes (`lint`, `check-types`, `test`, `build`, `audit`).
